...
| F | 受注APIのアクセストークン取得方法 | ✅ 完了（**OAuth2と判明。継続メンテが必要**） |
| B | 初回/継続判定を実装するかのご判断 | 🔲 ご判断待ち（開発と並行で可） |

**方式A（標準構成）・商品識別方式（ループタグ）ともに確定したため、実装（Phase 1）に着手できます。**
初回/継続判定（OAuth連携）は実装コストに見合うか検討が必要なため、
`docs/09-cart-integration.md` 3.7 節で判断材料を整理しました。
技術的な背景・確定した設定値は
