...
| 商品の識別方式 | **受注API の `order_detail[].product_code`**（購入明細）で識別。ページ URL からは商品を特定しない（`pm100` は商品コードではないと判明） |
| CV 連携方式 | サンクスページの軽量タグ（注文番号のみ）+ 受注API 同期バッチ。1 注文複数商品にも標準対応 |
...
顧客サイト（LP・商品ページ）        スマレジEC（サンクスページ・同一ドメイン）
   共通タグ（URL ベースの計測のみ）      共通タグ + 軽量CVタグ（注文番号のみ）
        │                                    │
...
- **商品はページ URL では識別しない。** サンクスページの軽量CVタグ（注文番号のみ）+
  **受注API 同期バッチ**が商品コード・金額・初回/継続を確定する
  （`pm100` のような URL 上の記号は使わない）
- 1 注文に複数商品が含まれる場合も、受注API の `order_detail[]`（配列）から
  明細単位で正確に取得するため標準対応
...
商品コード・金額・初回/継続の判定は、この後**受注API 同期バッチ**が
自動的に補完します（`09-cart-integration.md` 3 章）。
...
| C | 商品ページの URL と商品識別方法 | ✅ 完了（**受注API方式に確定**） |
| D | スマレジEC 受注APIの仕様 | ✅ 完了（**CV連携方式を再確定**） |
| B | APIアクセストークン・注文番号の変数名 | 🔲 後日回答予定（開発と並行で可） |

**方式A（標準構成）・商品識別方式（受注API）ともに確定したため、実装（Phase 1）に着手できます。**
技術的な背景・確定した設定値は
