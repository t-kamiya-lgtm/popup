...
- 広告タグ（クリックID）の引き継ぎ（`packages/sdk/src/render.ts` の `forwardQueryParams`）:
  バナークリック時、現在ページの URL クエリ文字列全体（`gclid`・`srsltid`・`utm_*` など）を
  リンク先 URL にそのまま引き継ぐ。特定のパラメータ名だけを対象にする方式（allowlist）
  ではなく丸ごと転送する設計 — 広告プラットフォームが独自に追加するクリックIDの
  種類は今後も増え続けるため、名前を列挙する方式だと新しいものを取りこぼす。
  リンク先 URL 自体に同名のパラメータが既に指定されている場合はそちらを優先。
  純粋関数のユニットテスト5件に加え、実ブラウザ（Playwright、closed Shadow DOM 越しの
  実クリック）でリクエストを直接検証し、`?gclid=abc123&utm_source=google` が
  実際の遷移先 URL に反映されることを確認済み
- 対象ページ URL ルールの正規化（`apps/web/lib/url-pattern.ts`）: 実際の本番キャンペーンで
  「対象ページ」ルールにブラウザのアドレスバーからコピーしたフル URL
  （`https://www.primedirect.jp/protein-pro01.html?force-page=sp` 等）を入力した結果、
  imp が1件も記録されない不具合が発生した。判定は `location.pathname` のみで行う仕様
  （オリジン・クエリ文字列は見ない）のため、フルURLはもちろん、末尾にクエリ文字列が
  付いているだけでも一致しない。保存時にオリジン・クエリ文字列・ハッシュを自動で
  取り除き、パス部分だけに正規化するよう管理画面（クライアント側）と
  `PATCH /api/v1/campaigns/[id]`（サーバ側、直接APIを叩いた場合の保証）の両方を修正。
  実ブラウザでフルURLを保存→パスだけに正規化された値が表示・保存されることを確認済み
