...
### 1.3 商品ページの商品コードタグ（プライムダイレクトでは不使用）

> **設計変更**（2026-08 確認結果を反映）: 当初はこの商品ページタグを
> 商品別レポートの主軸にする想定でしたが、プライムダイレクトでは
> 商品ページ URL に確実な商品コードが存在しない（`/products/pm100/` の
> `pm100` は実際の商品コードではない）ことが判明したため、**このタグは使いません**。
>
> 商品の識別は、代わりに**サンクスページの CV タグが受け取る
> 「カートに入った実際の商品コード」**（差し込み変数由来）で行います
> （1.2 節・`09-cart-integration.md` 4 章）。商品ページ・LP 側のページ分類は
> 従来どおり `page_groups` の URL ルールで行い、これはエンゲージメント
> （imp / click）の集計軸として使います。

このタグ（`pqz.push(['page', {productCode}])`）自体は仕組みとして残しておき、
将来 他サイト導入時にページへ確実な商品コードを埋め込める場合に使えるようにします。
...
flowchart TD
  ...
  B2 -- No --> C[URL を pageGroup 判定]
  C --> D[配信期間 / デバイス / オーディエンス 判定]
  D --> E[targets URL の include・exclude 判定]
...
配信判定は URL ルールのみに基づくため、商品コードタグの到着を待つ必要がなくなり、
**判定を即座に開始**できます（設定 JSON 取得直後）。
