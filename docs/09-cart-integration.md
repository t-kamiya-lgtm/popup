...
| 2b | 差し込み変数の**正確な変数名**（注文番号 / 金額 / 商品コード / 初回・継続の別） | CV タグの実装に直結 | 🔲 要取得 |
| 3 | カート / サンクスページの**ホスト名** | 別ホストならリンク装飾が必須。ビュースルー CV は不可 | 🔲 **要確認（最重要）** |
| 4 | カートページ URL に**クエリパラメータを付与して遷移できるか**（消されないか） | `pz_t` による接触情報の引き継ぎ可否 | 🔲 要確認（3 が別ホストの場合のみ） |
| 5 | **カゴ落ちメール等からの復帰導線**でパラメータが失われないか | 復帰 CV の帰属精度 | 🔲 要確認 |
| 6 | 商品ページ / LP は `www.primedirect.jp` 側か | タグ設置箇所とターゲティング設計 | 🔲 要確認 |
| 7 | **受注データの API / CSV エクスポート**が可能か | S2S CV のフォールバック手段 | 🔲 要確認 |
| 8 | 定期購入の **2 回目以降の受注**も CV タグが発火するか | CV の定義（下記 3 章） | 🔲 要確認 |

> #1・#2 が確定したことで、**方式C（S2S 連携）は不要**になりました。
> 残る分岐は **#3 のホスト名のみ**（方式A か 方式B か）です。

#### #3 の確認方法（所要 1 分）

1. `https://www.primedirect.jp/` で商品をカートに入れ、購入手続きに進む
2. **アドレスバーのホスト名**を各ステップで確認する
   - カート画面
   - お客様情報入力画面
   - 注文確認画面
   - ご注文完了（サンクス）画面
...
| 結果 | 採用方式 |
| --- | --- |
| 全ステップが `www.primedirect.jp` | **方式A**（ビュースルー CV も計測可・最も高精度） |
| 途中から別ホスト（例 `secure.〜` / カート事業者ドメイン） | **方式B**（リンク装飾。ビュースルー CV は計測不可） |

> 別ホストだった場合でも、**そのホストに独自ドメイン（例 `cart.primedirect.jp`）を
> 設定できないかを先に確認する価値があります**。同一ドメインにできれば計測精度が上がり、
> 実装も単純になります。スマレジ・リピート側の設定で可能なケースがあります。

#### 参考: 公開情報から分かった範囲

本設計の作成時点で外部から確認できた URL 構造（実機確認による裏取りが必要）:
...
- 上記はいずれも `www.primedirect.jp` 配下。**購入フロー（カート〜サンクス）が
  同一ホストかどうかは未確認**です
- 商品ページの URL 形式（`/products/detail.php?product_id=` 形式か等）は #6 で確認します。
...
flowchart TD
  ...
  A -->|No| S["方式C: S2S 連携<br/>← 不要になった"]
  B -->|Yes| C["方式A: 標準<br/>クリックスルー + ビュースルー"]
  B -->|No| D["方式B: リンク装飾<br/>クリックスルーのみ"]
  style S stroke-dasharray: 5 5
```

タグ設置と差し込み変数が使えることが確定したため、**方式C は保険として残すのみ**。
実装は方式A / 方式B のどちらかになります。
...
```jsonc
{
  ...
  "cartHosts": [],              // #3 の確認結果で追加（別ホストの場合）
  "crossDomainCart": false,     // #3 の確認結果で確定
  "timezone": "Asia/Tokyo",
  "cvClickWindowDays": 7,
  "cvViewWindowDays": 1,
...
配信対象から**既定で除外**するページ（購入動線を妨げないため）:

- カート・お客様情報入力・注文確認・ご注文完了
