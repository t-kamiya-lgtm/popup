...
差し込みタグ一覧を頂いたことで、確認事項はかなり絞られました。

| # | 確認事項 | 備考 |
| --- | --- | --- |
| 1 | `{注文番号}` の値（例 `165`）は、受注API の `order.order_id`・`order.ec_order_id` の**どちらと一致するか** | 3.4 節の突合キーの選定に必須 |
| 2 | **API アクセストークンの発行方法**（管理画面のどこで発行するか） | 実装に必須（初回/継続判定のみに使用） |
| 3 | `order.order_cnt`（購入回数）は「その顧客の通算注文回数」で合っているか | 初回/継続判定の正確性に関わる |
| 4 | 「初回 / 継続（2回目以降）」を示す差し込みタグが、頂いた一覧以外に存在しないか | あれば受注API 連携自体が不要になり、さらに単純化できる |
| 5 | 受注APIの呼び出しレート制限（任意） | 同期バッチの間隔設計に影響（数分おき程度なら通常問題にならない想定） |

> 1 と 4 が分かれば、実装にほぼ支障がなくなります。
> API 発行画面のスクリーンショットや、テスト用の `access_token` を頂ければ
> 3.4 節の突合ロジックの動作確認まで一気に進められます。
...
4. 管理画面で**受注API のアクセストークン**を登録（初回/継続判定のみに使用。3.5 節）
5. ページグループを設定（`/products/` 配下・`/protein.html` 等の URL ルール）
6. 管理画面の「タグ設置チェッカー」で受信確認
   - 共通タグ: 受信あり / なし
   - CV タグ: 受信あり / なし・商品明細（`items`）を検出した件数
   - 受注API 同期（初回/継続判定）: 直近の同期成功時刻・突合成功件数
7. テスト表示（`?pz_preview=` 付き URL）で見た目を確認
8. テスト購入を 1 件行い、CV と商品明細が**即座に**計上されることを確認
   （数分後に初回/継続の判定が確定することもあわせて確認）→ 配信開始
