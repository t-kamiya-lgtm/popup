...
### 3.7 検討結果: OAuth 連携を Phase 1 で実装（確定）

差し込みタグ一覧を再確認いただいた結果、「初回/継続」に相当するタグは
**見当たらない**とのことでした。また、アプリ登録の手続き自体は
負担が大きくないとのご判断のため、**OAuth連携を Phase 1 で実装します。**

検討時に並べた選択肢と、それぞれの判断根拠:

| 案 | 内容 | 採否 |
| --- | --- | --- |
| C. 差し込みタグで代替 | 「初回購入」等の別名タグの有無を確認 | ❌ 該当タグなしと確認済み |
| B. Phase 1 は判定なし | 全 CV をまとめて集計、判定は Phase 2 へ先送り | ❌ 見送り（初回獲得の効果測定を Phase 1 から行う） |
| **A. OAuth連携を実装する** | 3.5 節のとおり | ✅ **採用（確定）** |

実装スコープは 3.5 節（OAuth2 フロー）・4.5 節（サイト設定値）・
`03-data-model.md`（`site_order_api_connections` テーブル）・
`04-api.md` 1.5 節（連携用エンドポイント）・
`06-admin.md` 6.5 節（連携画面）のとおりです。
...
```jsonc
{
  ...
  "orderApi": {
    "enabled": true,            // ✅ 確定: Phase 1 で OAuth 連携を実装（3.7 節）
