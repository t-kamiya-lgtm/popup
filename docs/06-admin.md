...
> 商品はページ URL ではなく、**サンクスページで受け取った「実際にカートに入った
> 商品コード」**で識別します（`09-cart-integration.md` 4 章）。
...
- 商品コードは CV タグの差し込み変数から受信した値をそのまま使うため、カート側の受注データと必ず一致する
- 「商品コード未受信」の行は、差し込み変数が空 or 未展開だった CV。
  0 件でなければ**タグ設置に不備がある兆候**として警告バナーを表示する
...
3. よくある設置ミスを診断
   ...
   - **CV タグの商品コードが空、または `{{` を含む未展開の値のまま届いている**
     → 差し込み変数名を確認するよう案内（商品別レポートに計上されなくなるため）
