...
> 商品はページ URL ではなく、**受注API（`order_detail[].product_code`）**で
> 識別します（`09-cart-integration.md` 3〜4 章）。
> そのため imp / click / CTR は持たず、**CV と売上のみ**を表示します。
> 「どのページでどれだけ見られ、クリックされたか」は 5.3.1 の**ページ別**で見ます。

| 商品コード | 商品名 | CV（クリックスルー） | CV（ビュースルー） | 売上 |
| --- | --- | ---: | ---: | ---: |
| PD-1001 | 定期コースA | 18 | 3 | ¥162,000 |
| PD-1002 | お試しセット | 9 | 1 | ¥81,000 |
| — | （受注API 未突合の CV） | 2 | 0 | ¥18,000 |

- 商品コードは受注API から取得した値をそのまま使うため、カート側の受注データと必ず一致する
- 「受注API 未突合の CV」の行は、サンクスページのタグは届いたが受注API との突合が
  8 日を超えて完了しなかった CV。0 件でなければ**同期に不備がある兆候**として警告バナーを表示する
...
```
✓ 共通タグ                       受信あり  最終 3分前
✓ CV タグ（仮登録）                受信あり  最終 12分前
✓ 受注API 同期                    正常     直近同期 4分前・突合成功 12/12件
```

（カートが別ドメインの構成では、上に「共通タグ（カートドメイン）」の行が追加される）

3. よくある設置ミスを診断
   - CV タグ（仮登録）はあるが `orderId` が空 → 重複計測の警告
   - 許可ホストに未登録のドメインからイベントが来ている → 追加を促す
   - **CV タグの `orderId` と受注API の `ec_order_id` が突合できない**
     → 差し込み変数名の確認、またはアクセストークンの権限を案内
   - **受注API 同期が失敗している**（トークン期限切れ・レート制限超過等）
     → 商品別レポートが更新されなくなるため、優先度高で警告表示
